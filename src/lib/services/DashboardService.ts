...
export class DashboardService implements IDashboardService {
  ...
  async getDashboardData(userId: string): Promise<DashboardData> {
    ...
    const [receiptCount, recipeCount, savedCount, activeList] =
      await Promise.all([
    ...
    const recentReceipts: ReceiptDao[] = await this.receiptRepo.findByUserId(
      userId,
      3,
    );
    ...
    const suggestions: SmartSuggestion[] =
      await this.shoppingListController.getSmartSuggestions(userId, 5);
