...
export class PgVectorService implements IVectorService {
  ...
  async search(
    ...
    const results = await db
      ...
      .where(
        sql`1 - (${cookbookEmbeddings.embedding} <=> ${embeddingStr}::vector) >= ${threshold}`,
      )
      .orderBy(
        desc(
          sql`1 - (${cookbookEmbeddings.embedding} <=> ${embeddingStr}::vector)`,
        ),
      )
      .limit(limit);
  ...
  async batchUpsert(
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map((doc) =>
          this.upsert(doc.id, doc.recipeTitle, doc.content, doc.metadata),
        ),
      );
    }
  }
