...
export { type DietType, type PreferenceLevel, type AllergySeverity, type TasteProfile, type CompatibilityResult };
...
export class TasteProfileService implements ITasteProfileService {
	...
	async setIngredientPreference(
		...
		await this.ingredientPrefRepo.create({ userId, ingredientName, preference });
	}
	...
	async checkRecipeCompatibility(userId: string, recipeId: string): Promise<CompatibilityResult> {
		...
		if (profile.dietType === 'vegan' || profile.dietType === 'vegetarian') {
			...
			const forbidden = profile.dietType === 'vegan'
	...
	private recipeContains(recipe: RecipeWithIngredientsDao, term: string): boolean {
		...
		// Check ingredients
		if (recipe.ingredients.some((i: RecipeIngredientDao) => i.name.toLowerCase().includes(normalizedTerm))) return true;
		// Check title
