<script lang="ts">
	let { width = 'w-32', color = 'white', rotate = '-rotate-1', class: className = '' } = $props();
...
<div
	aria-hidden="true"
	class={`
    relative z-20 h-8 overflow-hidden shadow-sm select-none
...
>
	...
	<div class="absolute top-0 bottom-0 left-[25%] w-px bg-black/5 blur-[0.5px]"></div>
	<div class="absolute top-0 right-[25%] bottom-0 w-0.5 bg-white/40 blur-[1px]"></div>
