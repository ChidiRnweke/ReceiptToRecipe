...
export class RecipeRepository implements IRecipeRepository {
  ...
  async findByIdWithIngredients(
    id: string,
  ): Promise<RecipeWithIngredientsDao | null> {
    const recipe = await this.db.query.recipes.findFirst({
      where: eq(schema.recipes.id, id),
  ...
  async countByReceiptIds(
    receiptIds: string[],
  ): Promise<RecipeCountByReceiptDao[]> {
  ...
  private toDao(recipe: typeof schema.recipes.$inferSelect): RecipeDao {
    return {
      ...
      imageUrl: recipe.imageUrl,
      imageStatus: recipe.imageStatus as
        | "QUEUED"
        | "PROCESSING"
        | "DONE"
        | "FAILED",
  ...
  private ingredientToDao(
    ing: typeof schema.recipeIngredients.$inferSelect,
  ): RecipeIngredientDao {
    return {
      id: ing.id,
...
export class SavedRecipeRepository implements ISavedRecipeRepository {
    ...
    return saved.map((s) => ({
      ...
      recipe: s.recipe
        ? {
            ...
            imageStatus: s.recipe.imageStatus as
              | "QUEUED"
              | "PROCESSING"
              | "DONE"
              | "FAILED",
  ...
  async create(savedRecipe: NewSavedRecipeDao): Promise<SavedRecipeDao> {
    ...
    if (!created) {
      ...
      const existing = await this.findByUserAndRecipe(
        savedRecipe.userId,
  ...
  private toDao(
    saved: typeof schema.savedRecipes.$inferSelect,
  ): SavedRecipeDao {
    return {
      id: saved.id,
      userId: saved.userId,
