...
export class ShoppingListRepository implements IShoppingListRepository {
  ...
  private toDao(
    list: typeof schema.shoppingLists.$inferSelect,
  ): ShoppingListDao {
    return {
      id: list.id,
      userId: list.userId,
  ...
  private itemToDao(
    item: typeof schema.shoppingListItems.$inferSelect,
  ): ShoppingListItemDao {
    return {
      id: item.id,
...
export class ShoppingListItemRepository implements IShoppingListItemRepository {
  ...
  async getMaxOrderIndex(listId: string): Promise<number> {
    const result = await this.db
      .select({
        maxOrder:
          sql<number>`COALESCE(MAX(${schema.shoppingListItems.orderIndex}), -1)`.as(
            "maxOrder",
          ),
      })
