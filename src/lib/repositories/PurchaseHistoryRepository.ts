...
import type { PurchaseHistoryDao, NewPurchaseHistoryDao, UpdatePurchaseHistoryDao, SmartSuggestionDao } from './daos';
...
export class PurchaseHistoryRepository implements IPurchaseHistoryRepository {
	...
	async create(history: NewPurchaseHistoryDao): Promise<PurchaseHistoryDao> {
		const [created] = await this.db.insert(schema.purchaseHistory).values({
	...
	async update(id: string, history: UpdatePurchaseHistoryDao): Promise<PurchaseHistoryDao> {
		const [updated] = await this.db.update(schema.purchaseHistory)
			.set({
				...history,
				updatedAt: new Date()
			})
