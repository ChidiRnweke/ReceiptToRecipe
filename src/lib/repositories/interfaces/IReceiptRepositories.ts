...
export interface IReceiptItemRepository {
	...
	createMany(items: import('../daos').NewReceiptItemDao[]): Promise<import('../daos').ReceiptItemDao[]>;
	update(id: string, item: import('../daos').UpdateReceiptItemDao): Promise<import('../daos').ReceiptItemDao>;
	delete(id: string): Promise<void>;
	findLatestByNormalizedName(userId: string, normalizedName: string): Promise<(import('../daos').ReceiptItemDao & { storeName: string | null; receiptId: string }) | null>;
}
