...
export interface IUserDietaryProfileRepository {
	...
	update(userId: string, profile: UpdateUserDietaryProfileDao): Promise<UserDietaryProfileDao>;
	delete(userId: string): Promise<void>;
}

export interface IUserAllergyRepository {
	findByUserId(userId: string): Promise<UserAllergyDao[]>;
	findByUserAndAllergen(userId: string, allergen: string): Promise<UserAllergyDao | null>;
	create(allergy: NewUserAllergyDao): Promise<UserAllergyDao>;
	update(userId: string, allergen: string, severity: string): Promise<UserAllergyDao>;
...
export interface IUserIngredientPreferenceRepository {
	...
	findByUserAndIngredient(userId: string, ingredientName: string): Promise<UserIngredientPreferenceDao | null>;
	create(preference: NewUserIngredientPreferenceDao): Promise<UserIngredientPreferenceDao>;
	update(userId: string, ingredientName: string, preference: string): Promise<UserIngredientPreferenceDao>;
...
export interface IUserCuisinePreferenceRepository {
	...
	findByUserAndCuisine(userId: string, cuisineType: string): Promise<UserCuisinePreferenceDao | null>;
	create(preference: NewUserCuisinePreferenceDao): Promise<UserCuisinePreferenceDao>;
	update(userId: string, cuisineType: string, preference: string): Promise<UserCuisinePreferenceDao>;
