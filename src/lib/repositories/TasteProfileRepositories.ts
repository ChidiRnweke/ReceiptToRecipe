...
export class UserDietaryProfileRepository implements IUserDietaryProfileRepository {
	...
	async create(profile: NewUserDietaryProfileDao): Promise<UserDietaryProfileDao> {
		const [created] = await this.db.insert(schema.userDietaryProfiles).values({
	...
	async update(userId: string, profile: UpdateUserDietaryProfileDao): Promise<UserDietaryProfileDao> {
		const [updated] = await this.db.update(schema.userDietaryProfiles)
			.set({
				dietType: profile.dietType ?? null,
				updatedAt: new Date()
			})
	...
	async delete(userId: string): Promise<void> {
		await this.db.delete(schema.userDietaryProfiles)
			.where(eq(schema.userDietaryProfiles.userId, userId));
	}

	private toDao(profile: typeof schema.userDietaryProfiles.$inferSelect): UserDietaryProfileDao {
		return {
			id: profile.id,
			userId: profile.userId,
...
export class UserAllergyRepository implements IUserAllergyRepository {
	...
	async findByUserAndAllergen(userId: string, allergen: string): Promise<UserAllergyDao | null> {
	...
	async create(allergy: NewUserAllergyDao): Promise<UserAllergyDao> {
		const [created] = await this.db.insert(schema.userAllergies)
			.values({
	...
	async update(userId: string, allergen: string, severity: string): Promise<UserAllergyDao> {
		const [updated] = await this.db.update(schema.userAllergies)
			.set({ severity })
			.where(and(
				eq(schema.userAllergies.userId, userId),
				eq(schema.userAllergies.allergen, allergen)
	...
	async delete(userId: string, allergen: string): Promise<void> {
		await this.db.delete(schema.userAllergies)
			.where(and(
				eq(schema.userAllergies.userId, userId),
				eq(schema.userAllergies.allergen, allergen)
			));
	...
	private toDao(allergy: typeof schema.userAllergies.$inferSelect): UserAllergyDao {
		return {
			id: allergy.id,
			userId: allergy.userId,
			allergen: allergy.allergen,
...
export class UserIngredientPreferenceRepository implements IUserIngredientPreferenceRepository {
	...
	async findByUserAndIngredient(userId: string, ingredientName: string): Promise<UserIngredientPreferenceDao | null> {
	...
	async create(preference: NewUserIngredientPreferenceDao): Promise<UserIngredientPreferenceDao> {
		const [created] = await this.db.insert(schema.userIngredientPreferences)
			.values({
			...
			.onConflictDoUpdate({
				target: [schema.userIngredientPreferences.userId, schema.userIngredientPreferences.ingredientName],
	...
	async update(userId: string, ingredientName: string, preference: string): Promise<UserIngredientPreferenceDao> {
		const [updated] = await this.db.update(schema.userIngredientPreferences)
			.set({ preference })
			.where(and(
				eq(schema.userIngredientPreferences.userId, userId),
				eq(schema.userIngredientPreferences.ingredientName, ingredientName)
	...
	async delete(userId: string, ingredientName: string): Promise<void> {
		await this.db.delete(schema.userIngredientPreferences)
			.where(and(
				eq(schema.userIngredientPreferences.userId, userId),
				eq(schema.userIngredientPreferences.ingredientName, ingredientName)
			));
	...
	private toDao(preference: typeof schema.userIngredientPreferences.$inferSelect): UserIngredientPreferenceDao {
		return {
			id: preference.id,
			userId: preference.userId,
			ingredientName: preference.ingredientName,
...
export class UserCuisinePreferenceRepository implements IUserCuisinePreferenceRepository {
	...
	async findByUserAndCuisine(userId: string, cuisineType: string): Promise<UserCuisinePreferenceDao | null> {
	...
	async create(preference: NewUserCuisinePreferenceDao): Promise<UserCuisinePreferenceDao> {
		const [created] = await this.db.insert(schema.userCuisinePreferences)
			.values({
				userId: preference.userId,
			...
			.onConflictDoUpdate({
				target: [schema.userCuisinePreferences.userId, schema.userCuisinePreferences.cuisineType],
				set: { preference: preference.preference }
			})
			.returning();
		return this.toDao(created);
	...
	async update(userId: string, cuisineType: string, preference: string): Promise<UserCuisinePreferenceDao> {
		const [updated] = await this.db.update(schema.userCuisinePreferences)
			.set({ preference })
			.where(and(
				eq(schema.userCuisinePreferences.userId, userId),
				eq(schema.userCuisinePreferences.cuisineType, cuisineType)
	...
	async delete(userId: string, cuisineType: string): Promise<void> {
		await this.db.delete(schema.userCuisinePreferences)
			.where(and(
				eq(schema.userCuisinePreferences.userId, userId),
				eq(schema.userCuisinePreferences.cuisineType, cuisineType)
			));
	...
	private toDao(preference: typeof schema.userCuisinePreferences.$inferSelect): UserCuisinePreferenceDao {
		return {
			id: preference.id,
			userId: preference.userId,
			cuisineType: preference.cuisineType,
