...
export const load: PageServerLoad = async ({ locals }) => {
    ...
    const controller = new TasteProfileController(AppFactory.getTasteProfileService());
...
export const actions: Actions = {
    setDiet: async ({ locals, request }) => {
        ...
        const finalDiet = dietTypeRaw === 'null' || !dietTypeRaw ? null : (dietTypeRaw as DietType);
    ...
    addAllergy: async ({ locals, request }) => {
        ...
        try {
            const controller = new TasteProfileController(AppFactory.getTasteProfileService());
            await controller.addAllergy(locals.user.id, allergen, severity || 'avoid');
            return { success: true };
        } catch (e) {
    ...
    setIngredientPref: async ({ locals, request }) => {
        ...
        try {
            const controller = new TasteProfileController(AppFactory.getTasteProfileService());
            await controller.setIngredientPreference(locals.user.id, ingredient, preference);
            return { success: true };
        } catch (e) {
            return fail(500);
        }
    ...
    setCuisinePref: async ({ locals, request }) => {
        ...
        try {
            const controller = new TasteProfileController(AppFactory.getTasteProfileService());
            await controller.setCuisinePreference(locals.user.id, cuisine, preference);
            return { success: true };
        } catch (e) {
            return fail(500);
        }
