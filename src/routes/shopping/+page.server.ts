...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  if (recipeIds.size > 0) {
    ...
    recipeMap = Object.fromEntries(
      recipeList.map((r) => [r.id, { id: r.id, title: r.title }]),
    );
  }
...
export const actions: Actions = {
  ...
  createList: async ({ request, locals }) => {
    ...
    try {
      const shoppingListController = AppFactory.getShoppingListController();
      const list = await shoppingListController.createList(
        locals.user.id,
        name,
      );
    ...
    try {
      ...
      return fail(500, {
        error:
          error instanceof Error
            ? error.message
            : "Failed to generate restock list",
      });
    }
  },
    ...
    try {
      ...
      if (!skipPantryCheck) {
        ...
        if (pantryMatch) {
          ...
          const dateStr = pantryMatch.lastPurchased.toLocaleDateString(
            "en-US",
