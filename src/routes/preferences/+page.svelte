...
<div class="mx-auto max-w-3xl space-y-6">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-ink-light">
          Tell us how you like to eat, and we'll tailor every recipe
        </p>
      </div>
    </div>
  </div>
  ...
  >
        ...
        <div class="flex flex-wrap gap-2">
          {#each commonAllergies as allergy}
            <button
              type="button"
              onclick={() =>
                toggleItem(allergies, allergy, (v) => (allergies = v))}
              class="rounded-full border px-3 py-1 text-sm transition-colors {allergies.includes(
                allergy,
              )
              ...
              {#if !commonAllergies.includes(allergy)}
                <Badge variant="destructive" class="gap-1">
                  ...
                  <button
                    type="button"
                    onclick={() =>
                      removeItem(allergies, i, (v) => (allergies = v))}
                  >
                    <X class="h-3 w-3" />
                  </button>
                </Badge>
      ...
      <Card.Content class="space-y-4">
        <div class="flex flex-wrap gap-2">
          {#each commonDiets as diet}
            <button
              ...
              class="rounded-full border px-3 py-1 text-sm transition-colors {dietaryRestrictions.includes(
                diet,
              )
        ...
        <div class="flex gap-2">
          <Input
            ...
            onkeydown={(e) =>
              e.key === "Enter" &&
              (e.preventDefault(), addDietaryRestriction())}
          />
          <Button
            type="button"
            variant="outline"
            ...
            <button
              ...
              onclick={() =>
                toggleItem(
                  cuisinePreferences,
                  cuisine,
                  (v) => (cuisinePreferences = v),
                )}
              class="rounded-full border px-3 py-1 text-sm transition-colors {cuisinePreferences.includes(
                cuisine,
              )
                ? 'border-sage-500 bg-sage-100 text-sage-700'
                : 'border-sand bg-paper hover:border-sage-400'}"
      ...
      <Card.Content class="space-y-4">
        <div class="flex gap-2">
          <Input
            ...
            onkeydown={(e) =>
              e.key === "Enter" &&
              (e.preventDefault(), addExcludedIngredient())}
          />
          <Button
            type="button"
            variant="outline"
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="space-y-2">
            ...
            <p class="text-xs text-ink-muted">
              Recipes will aim for ~1/3 of this per meal
            </p>
          </div>
          <div class="space-y-2">
            ...
            />
            <p class="text-xs text-ink-muted">
              Default number of servings for generated recipes
            </p>
          </div>
        </div>
      </Card.Content>
    ...
    <!-- Submit -->
    <Card.Root
      class="sticky bottom-4 bg-linear-to-r from-sage-50 to-paper shadow-lg"
    >
