...
export const actions: Actions = {
  default: async ({ request, locals }) => {
    ...
    const dietaryRestrictions = parseStringList(
      data.get("dietaryRestrictions")?.toString(),
      {
        maxItems: 20,
        maxLength: 48,
      },
    );
    const cuisinePreferences = parseStringList(
      data.get("cuisinePreferences")?.toString(),
      {
        maxItems: 20,
        maxLength: 48,
      },
    );
    const excludedIngredients = parseStringList(
      data.get("excludedIngredients")?.toString(),
      {
        maxItems: 20,
        maxLength: 48,
      },
