...
<div class="flex min-h-screen flex-col items-center justify-center bg-[#fdfaf6] p-4 relative overflow-hidden">
  <!-- Desk Texture -->
  <div
  ...
  <div class="absolute bottom-10 right-10 opacity-5 -rotate-12 pointer-events-none">
  ...
  <div class="relative z-10 w-full max-w-md">
    ...
    <div class="mb-8 text-center">
      <div class="relative mx-auto w-fit mb-6">
        <div class="flex h-20 w-20 items-center justify-center rounded-full bg-white border border-stone-200 shadow-sm">
          <ChefHat class="h-10 w-10 text-stone-700" />
        </div>
      </div>
      <h1 class="font-display text-4xl text-stone-800 font-bold tracking-tight">Receipt to Recipe</h1>
      <p class="mt-3 font-serif text-stone-600 italic text-lg">Turn your grocery receipts into delicious meals.</p>
    </div>

    <!-- Signup Card -->
    <div class="relative bg-white rounded-lg shadow-xl border border-stone-200 p-8 md:p-10">
      <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-amber-400 to-orange-500 rounded-t-lg"></div>

      <div class="relative z-10 space-y-6">
        <div class="text-center space-y-2">
          {#if form?.userExists}
            <h2 class="text-xl font-semibold text-stone-800">You already have an account!</h2>
            <p class="text-stone-500 text-sm">It looks like that email is already registered.</p>
          {:else}
            <h2 class="text-xl font-semibold text-stone-800">Join the Waitlist</h2>
            <p class="text-stone-500 text-sm">We're currently in private beta. Sign up to get early access.</p>
          {/if}
        </div>
        ...
        {:else}
          ...
          <div class="pt-4 text-center">
               <a href="/login" class="text-xs text-stone-400 hover:text-stone-600 transition-colors">
