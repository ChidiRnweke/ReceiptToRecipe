...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const isSaved = viewerId
    ? await recipeController.isSaved(viewerId, recipe.id)
    : false;
  ...
  try {
    ...
    const recipeForAi = {
      ...
      ingredients: recipe.ingredients.map((i: any) => ({
        name: i.name,
        quantity:
          typeof i.quantity === "string" ? parseFloat(i.quantity) : i.quantity,
        unit: i.unit,
...
export const actions: Actions = {
  ...
  togglePublic: async ({ locals, params }) => {
    ...
    const isPublic = await recipeController.togglePublic(
      params.id,
      locals.user.id,
    );
  ...
  addToShopping: async ({ locals, params, request }) => {
    ...
    try {
      ...
      return fail(500, {
        error:
          err instanceof Error ? err.message : "Unable to add to shopping list",
      });
    }
  },
  ...
  adjustRecipe: async ({ locals, params, request }) => {
    ...
    try {
      await recipeController.adjustRecipeWithAi(
        params.id,
        locals.user.id,
        instruction.trim(),
      );
