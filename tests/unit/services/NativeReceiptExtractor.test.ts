...
describe('NativeReceiptExtractor', () => {
	...
	describe('extractReceipt', () => {
		...
		it('should convert local file path to base64', async () => {
			...
			expect(readFile).toHaveBeenCalledWith(expect.stringContaining('/uploads/receipt.png'));
	...
	describe('extractReceiptFromBuffer', () => {
		it('should create correct data URL from buffer', async () => {
			...
			const call = mockFetch.mock.calls[0];
			const body = JSON.parse(call[1].body);
			expect(body.document.image_url).toBe('data:image/webp;base64,dGVzdC1idWZmZXItZGF0YQ==');
		});
	});
	...
	describe('processOcrRequest', () => {
		it('should call Mistral OCR API with correct headers', async () => {
			...
			const result = await service.extractReceipt('https://example.com/receipt.jpg');
		...
		it('should throw on API error', async () => {
			...
			await expect(service.extractReceipt('https://example.com/receipt.jpg')).rejects.toThrow(
				'Mistral OCR API failed with status 401: Unauthorized'
			);
		});
		...
		it('should throw when no annotation present', async () => {
			...
			await expect(service.extractReceipt('https://example.com/receipt.jpg')).rejects.toThrow(
				'OCR failed: No annotation in response'
			);
		});
		...
		it('should pass JSON schema in request', async () => {
			...
			expect(body.document_annotation_format.json_schema.name).toBe('receipt_extraction');
		});
	});
});
