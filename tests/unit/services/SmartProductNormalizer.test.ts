...
describe('SmartProductNormalizer', () => {
  ...
  describe('normalizeProduct', () => {
        ...
        choices: [
          {
            message: {
              content: [
                ...
                { type: 'text', text: '{"normalizedName": "Milk", "productGroup": "Dairy", "category": "dairy"}' }
              ]
            }
          }
        ]
    ...
    it('should include system prompt in request', async () => {
      mockResponse({ normalizedName: 'Test', productGroup: 'Other', category: 'other' });
    ...
    it('should request JSON format response', async () => {
      ...
      const call = mocks.send.mock.calls[0][0];
      expect(call.chatGenerationParams.responseFormat).toEqual({ type: 'json_object' });
    });
  });
});
