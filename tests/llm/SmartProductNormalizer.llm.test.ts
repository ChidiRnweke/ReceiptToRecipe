  ...
  describe("normalizeProduct", () => {
    it("should normalize a branded soda product", async () => {
      const result = await withRetry(() =>
        service.normalizeProduct("Coca Cola Zero 33cl"),
      );
    ...
    it("should normalize a fresh produce item", async () => {
      const result = await withRetry(() =>
        service.normalizeProduct("Organic Gala Apples 1kg"),
      );
    ...
    it("should normalize a dairy product", async () => {
      const result = await withRetry(() =>
        service.normalizeProduct("Arla Full Fat Milk 1L"),
      );
    ...
    it("should normalize a bakery item", async () => {
      ...
      expect(result.category.toLowerCase()).toMatch(
        /bakery|bread|pantry|grain/,
      );
    });

    it("should handle ambiguous receipt shorthand", async () => {
      const result = await withRetry(() =>
        service.normalizeProduct("ORG BAN 1.2KG"),
      );
    ...
    it("should return consistent structure for unknown products", async () => {
      const result = await withRetry(() =>
        service.normalizeProduct("MISC ITEM 12345"),
      );
    ...
    it("should normalize multiple items consistently", async () => {
      ...
      expect(result1.category.toLowerCase()).toBe(
        result2.category.toLowerCase(),
      );
    });
  });
});
