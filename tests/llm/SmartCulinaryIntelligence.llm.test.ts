...
describeLLM('SmartCulinaryIntelligence (LLM Integration)', () => {
	...
	beforeAll(() => {
		service = new SmartCulinaryIntelligence(OPENROUTER_API_KEY, CHEAP_CHAT_MODEL);
	});

	describe('chat', () => {
		it('should return a non-empty string response', async () => {
			const result = await withRetry(() =>
				service.chat([{ role: 'user', content: 'Reply with exactly one word: hello' }])
			);
	...
	describe('normalizeIngredient', () => {
		it('should parse a weight-based ingredient', async () => {
			const result = await withRetry(() => service.normalizeIngredient('1lb chicken breast'));
		...
		it('should parse a volume-based ingredient', async () => {
			const result = await withRetry(() => service.normalizeIngredient('2 cups flour'));
		...
		it('should parse a count-based ingredient', async () => {
			const result = await withRetry(() => service.normalizeIngredient('3 eggs'));
		...
		it('should handle fractional quantities', async () => {
			const result = await withRetry(() => service.normalizeIngredient('1/2 cup sugar'));
	...
	describe('embed', () => {
		it('should return a numeric array embedding', async () => {
			const result = await withRetry(() => service.embed('chicken parmesan recipe'));
	...
	describe('generateRecipe', () => {
		it('should generate a valid recipe from ingredients', async () => {
			const context: RecipeContext = {
				availableIngredients: ['chicken breast', 'rice', 'garlic', 'soy sauce', 'broccoli'],
				preferences: {},
				servings: 2
			};
		...
		it('should respect cuisine hint', async () => {
			...
			const hasIndianElement = indianIndicators.some((word) => recipeText.includes(word));
		...
		it('should respect allergy constraints via taste profile', async () => {
			...
			expect(ingredientNames.every((name) => !name.includes('peanut'))).toBe(true);
		});
	});
	...
	describe('adjustRecipe', () => {
		const baseRecipe: GeneratedRecipe = {
			...
			instructions: 'Step 1: Cut chicken.\nStep 2: Stir fry with vegetables.\nStep 3: Add sauce.',
			servings: 2,
			prepTime: 10,
			cookTime: 15,
			cuisineType: 'Asian',
		...
		it('should modify a recipe based on instruction', async () => {
			const adjusted = await withRetry(() =>
				service.adjustRecipe(baseRecipe, 'Make it vegetarian by replacing chicken with tofu')
			);
			...
			const ingredientNames = adjusted.ingredients.map((i) => i.name.toLowerCase());
	...
	describe('suggestModifications', () => {
		it('should return an array of short suggestion strings', async () => {
			...
			const suggestions = await withRetry(() => service.suggestModifications(recipe));
