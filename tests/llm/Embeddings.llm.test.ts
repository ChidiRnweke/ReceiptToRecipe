...
describeLLM('Embeddings for RAG (LLM Integration)', () => {
  ...
  it('should produce embeddings that group similar recipes together', async () => {
    const [pastaEmb, pizzaEmb, sushiEmb] = await Promise.all([
      withRetry(() => service.embed('Classic spaghetti carbonara with pancetta and egg')),
      withRetry(() => service.embed('Homemade margherita pizza with mozzarella and basil')),
      withRetry(() => service.embed('Traditional Japanese sushi roll with fresh salmon')),
    ]);
