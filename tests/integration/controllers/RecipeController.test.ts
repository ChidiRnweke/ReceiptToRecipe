...
describe("RecipeController", () => {
  ...
  describe("generateRecipe", () => {
    ...
    it("should combine ingredientIds and customIngredients", async () => {
      ...
      const original = mockCulinaryIntelligence.generateRecipe.bind(
        mockCulinaryIntelligence,
      );
    ...
    it("should persist recipe and ingredients to repositories", async () => {
      ...
      mockCulinaryIntelligence.setMockRecipe(
        "parmesan,pasta,zucchini",
        mockRecipe,
      );
      ...
      const ingredients = await mockRecipeIngredientRepository.findByRecipeId(
        result.id,
      );
  ...
  describe("getRecipeWithServings", () => {
    it("should scale ingredient quantities", async () => {
      ...
      const result = await controller.getRecipeWithServings(
        recipe.id,
        4,
        userId,
      );
  ...
  describe("togglePublic", () => {
    ...
    it("should throw for non-existent recipe", async () => {
      await expect(controller.togglePublic("nope", userId)).rejects.toThrow(
        "Recipe not found",
      );
    });
    ...
    it("should throw for recipe owned by another user", async () => {
      ...
      await expect(controller.togglePublic(recipe.id, userId)).rejects.toThrow(
        "Recipe not found",
      );
    });
  });
  ...
  describe("deleteRecipe", () => {
    ...
    it("should throw for recipe not found", async () => {
      await expect(controller.deleteRecipe("nope", userId)).rejects.toThrow(
        "Recipe not found",
      );
    });
    ...
    it("should throw for recipe owned by another user", async () => {
      ...
      await expect(controller.deleteRecipe(recipe.id, userId)).rejects.toThrow(
        "Recipe not found",
      );
    });
  });
  ...
  describe("updateRecipe", () => {
    ...
    it("should replace ingredients when provided", async () => {
      ...
      const ings = await mockRecipeIngredientRepository.findByRecipeId(
        recipe.id,
      );
