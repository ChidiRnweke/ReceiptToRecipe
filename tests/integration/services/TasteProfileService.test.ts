...
} from '../../mocks/MockRecipeRepositories';
...
describe('TasteProfileService', () => {
	...
	describe('getUserTasteProfile', () => {
		...
		it('should aggregate data from all repositories', async () => {
			...
			await ingredientPrefRepo.create({
				userId,
				ingredientName: 'garlic',
				preference: 'love'
			});
			await cuisinePrefRepo.create({
				userId,
				cuisineType: 'Italian',
				preference: 'love'
			});
			...
			expect(profile.allergies).toContainEqual({
				allergen: 'peanuts',
				severity: 'severe'
			});
			expect(profile.allergies).toContainEqual({
		...
		it('should not include other users data', async () => {
			await dietaryProfileRepo.create({
				userId: 'other-user',
				dietType: 'vegan'
			});
			await allergyRepo.create({
	...
	describe('checkRecipeCompatibility', () => {
		...
		it('should penalize avoided ingredients', async () => {
			...
			expect(result.warnings.some((w) => w.includes('Mushrooms') && w.includes('avoid'))).toBe(
				true
			);
			expect(result.matchScore).toBeLessThan(100);
		});
		...
		it('should throw when recipe is not found', async () => {
			await expect(service.checkRecipeCompatibility(userId, 'non-existent-recipe')).rejects.toThrow(
				'Recipe not found'
			);
		});
