...
describe('DashboardService', () => {
	...
	describe('getDashboardData', () => {
		...
		it('should return correct receipt count and recent receipts', async () => {
			...
			const receipts = [];
			for (let i = 0; i < 5; i++) {
				receipts.push(await receiptRepo.create({
					userId,
		...
		it('should return correct recipe count and recent recipes with ingredients', async () => {
			...
			expect(data.recentRecipes[0].ingredients[0].name).toContain('Ingredient A');
		});
		...
		it('should aggregate data from multiple sources correctly', async () => {
			...
			await shoppingListItemRepo.createMany([
				{ shoppingListId: list.id, name: 'Apples', checked: false, orderIndex: 0 },
				{ shoppingListId: list.id, name: 'Oranges', checked: true, orderIndex: 1 }
			]);
			...
			const coffeeSuggestion = data.suggestions.find(s => s.itemName === 'Coffee');
		...
		it('should return 100% completion when all items are checked', async () => {
			...
			await shoppingListItemRepo.createMany([
				{ shoppingListId: list.id, name: 'Item A', checked: true, orderIndex: 0 },
				{ shoppingListId: list.id, name: 'Item B', checked: true, orderIndex: 1 }
			]);
		...
		it('should return pantry items sorted by stock confidence descending', async () => {
			...
			const freshIdx = data.pantry.findIndex(p => p.itemName === 'Fresh Item');
			const olderIdx = data.pantry.findIndex(p => p.itemName === 'Older Item');
