...
export class MockShoppingListRepository implements IShoppingListRepository {
  ...
  async findByIdWithItems(
    id: string,
  ): Promise<ShoppingListWithItemsDao | null> {
  ...
  async findActiveByUserId(
    ...
    const list = Array.from(this.lists.values()).find(
      (l) => l.userId === userId && l.isActive,
    );
    if (!list) return null;
  ...
  async findByUserId(userId: string): Promise<ShoppingListWithItemsDao[]> {
    ...
    return Promise.all(lists.map((l) => this.findByIdWithItems(l.id))).then(
      (results) => results.filter((l): l is ShoppingListWithItemsDao => !!l),
    );
  }
