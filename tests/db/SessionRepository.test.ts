...
import { UserRepository, SessionRepository } from '../../src/lib/repositories/UserRepositories';
...
describe('SessionRepository (DB)', () => {
  ...
  describe('create', () => {
    it('should create a session with the given ID and expiration', async () => {
      ...
      expect(Math.abs(session.expiresAt.getTime() - expiresAt.getTime())).toBeLessThan(1000);
    });
  });
  ...
  describe('updateExpiresAt', () => {
    it('should update the session expiration date', async () => {
      ...
      await sessionRepo.create({ id: 'extend-me', userId, expiresAt: originalExpiry });
      ...
      expect(found).not.toBeNull();
      expect(Math.abs(found!.expiresAt.getTime() - newExpiry.getTime())).toBeLessThan(1000);
    });
  });
});
