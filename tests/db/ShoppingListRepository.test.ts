...
describe('ShoppingListRepository (DB)', () => {
  ...
  describe('create', () => {
    ...
    it('should create an inactive list when specified', async () => {
      const list = await listRepo.create({ userId, name: 'Old List', isActive: false });
  ...
  describe('findById', () => {
    ...
    it('should return null for non-existent ID', async () => {
      const found = await listRepo.findById('00000000-0000-0000-0000-000000000000');
  ...
  describe('findByIdWithItems', () => {
    it('should return list with items sorted by orderIndex', async () => {
      ...
      await itemRepo.create({ shoppingListId: list.id, name: 'Bananas', orderIndex: 2 });
      await itemRepo.create({ shoppingListId: list.id, name: 'Apples', orderIndex: 0 });
      await itemRepo.create({ shoppingListId: list.id, name: 'Oranges', orderIndex: 1 });
    ...
    it('should return null for non-existent list', async () => {
      const found = await listRepo.findByIdWithItems('00000000-0000-0000-0000-000000000000');
      expect(found).toBeNull();
    });
  });
  ...
  describe('findActiveByUserId', () => {
    ...
    it('should include items sorted by orderIndex', async () => {
      const list = await listRepo.create({ userId, name: 'Active', isActive: true });
      await itemRepo.create({ shoppingListId: list.id, name: 'B', orderIndex: 1 });
      await itemRepo.create({ shoppingListId: list.id, name: 'A', orderIndex: 0 });
  ...
  describe('deactivateAllByUserId', () => {
    ...
    it('should not affect other users lists', async () => {
      ...
      await listRepo.create({ userId: otherUser.id, name: 'Their List', isActive: true });
  ...
  describe('ShoppingListItemRepository', () => {
    ...
    describe('createMany', () => {
      it('should create multiple items at once', async () => {
        ...
        expect(items.map(i => i.name).sort()).toEqual(['Apples', 'Bananas', 'Oranges']);
      });
    ...
    describe('findById', () => {
      it('should find an item by ID', async () => {
        const created = await itemRepo.create({ shoppingListId: listId, name: 'Find Me' });
    ...
    describe('findByListId', () => {
      it('should return items sorted by orderIndex', async () => {
        await itemRepo.create({ shoppingListId: listId, name: 'C', orderIndex: 2 });
        await itemRepo.create({ shoppingListId: listId, name: 'A', orderIndex: 0 });
        await itemRepo.create({ shoppingListId: listId, name: 'B', orderIndex: 1 });
    ...
    describe('findCheckedByListId', () => {
      it('should return only checked items', async () => {
        await itemRepo.create({ shoppingListId: listId, name: 'Checked', checked: true });
        await itemRepo.create({ shoppingListId: listId, name: 'Unchecked', checked: false });
        await itemRepo.create({ shoppingListId: listId, name: 'Also Checked', checked: true });
    ...
    describe('update', () => {
      it('should update an item', async () => {
        const item = await itemRepo.create({ shoppingListId: listId, name: 'Original' });
        const updated = await itemRepo.update(item.id, { name: 'Updated', checked: true });
    ...
    describe('delete', () => {
      it('should delete an item', async () => {
        const item = await itemRepo.create({ shoppingListId: listId, name: 'Delete Me' });
    ...
    describe('deleteCheckedByListId', () => {
      it('should delete only checked items from a list', async () => {
        await itemRepo.create({ shoppingListId: listId, name: 'Keep', checked: false });
        await itemRepo.create({ shoppingListId: listId, name: 'Remove 1', checked: true });
        await itemRepo.create({ shoppingListId: listId, name: 'Remove 2', checked: true });
      ...
      it('should not affect items in other lists', async () => {
        ...
        await itemRepo.create({ shoppingListId: listId, name: 'Check 1', checked: true });
        await itemRepo.create({ shoppingListId: otherList.id, name: 'Check 2', checked: true });
    ...
    describe('getMaxOrderIndex', () => {
      it('should return the max order index as a number', async () => {
        await itemRepo.create({ shoppingListId: listId, name: 'Item 1', orderIndex: 0 });
      ...
      it('should return the highest order index across multiple items', async () => {
        await itemRepo.create({ shoppingListId: listId, name: 'A', orderIndex: 0 });
        await itemRepo.create({ shoppingListId: listId, name: 'B', orderIndex: 5 });
        await itemRepo.create({ shoppingListId: listId, name: 'C', orderIndex: 3 });
    ...
    describe('updateOrderIndex', () => {
      it('should update the order index of an item', async () => {
        const item = await itemRepo.create({ shoppingListId: listId, name: 'Move Me', orderIndex: 0 });
