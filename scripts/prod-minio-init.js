...
async function run() {
	...
	const endPoint = secretMap['MINIO_ENDPOINT'] || process.env.MINIO_ENDPOINT || 'minio';
	const port = parseInt(secretMap['MINIO_PORT'] || process.env.MINIO_PORT || '9000');
	const useSSL = (secretMap['MINIO_USE_SSL'] || process.env.MINIO_USE_SSL) === 'true';
	const accessKey =
		minioCreds?.accessKey || secretMap['MINIO_ACCESS_KEY'] || process.env.MINIO_ACCESS_KEY;
	const secretKey =
		minioCreds?.secretKey || secretMap['MINIO_SECRET_KEY'] || process.env.MINIO_SECRET_KEY;
	const bucketName = secretMap['MINIO_BUCKET'] || process.env.MINIO_BUCKET || 'r2r-images';
