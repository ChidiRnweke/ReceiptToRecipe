...
export async function provisionDatabase() {
	...
	const adminInfisical = await connectToInfisical(adminClientId, adminClientSecret, siteUrl);

	console.log('Connecting to app Infisical project...');
	const appInfisical = await connectToInfisical(appClientId, appClientSecret, siteUrl);
	...
	adminSecrets.secrets.forEach((s) => (adminSecretMap[s.secretKey] = s.secretValue));
	...
	try {
		...
		const dbCheckResult = await adminClient.query('SELECT 1 FROM pg_database WHERE datname = $1', [
			dbName
		]);
		...
		const userCheckResult = await adminClient.query('SELECT 1 FROM pg_roles WHERE rolname = $1', [
			dbUser
		]);
